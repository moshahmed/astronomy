...
static bool g_verbose = false;      // true means produce debug messages
...
void printHeading(CalData& cd) {
  ...
  char title[256];
  sprintf( title,
      "%s, %d at latitude %5.2f %c, longitude %5.2f %c, tz %s%2.1f",
       monthNames[ cd.month-1 ], cd.year,
       cd.loc.degLatitude(),  cd.loc.degLatitude() >0? 'N':'S',
       cd.loc.degLongitude(), cd.loc.degLongitude()>0? 'W':'E',
       pUtc, cd.loc.timeZone() );
...
void usage(const char* pn, bool x = true)
{
  fprintf( stderr,
      ...
      "       month=0 for current month and year\n"
...
int main( int argc, char* *argv ) {
  ...
  sprintf( cfgFile, "%s%s", "DarkCal", CFG_EXT );
    ...
    if ( ConfigFile::OK != cf.filename( pCfgFile ) ) {
      ...
      if ( NULL != fp ) {
        fprintf( fp,
            ...
            "# e.g., Philadelphia, PA, US is longitude -75.16 W, latitude 39.95 N,\n"
            "#       and timeZone -5 West of UTC\n"
            "longitude=0.0\nlatitude=0.0\ntimeZone=0\n\n"
  ...
  if ( 0. == cd.loc.longitude() && 0. == cd.loc.latitude() )
    fprintf( stderr,
      "Latitude & Longitude are both set to 0. Is this what you intended?\n" );
  ...
  if ( 0 != pExt ) {
    char filename[32];
    sprintf( filename, "%04d-%02d.%s", cd.year, cd.month, pExt );
